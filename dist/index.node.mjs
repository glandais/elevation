...
})), J = /* @__PURE__ */ i({ NodeJsTileFetcher: () => Y }), Y, ee = r((() => {
	q(), Y = class {
		async fetchTile(n) {
			let r = await fetch(n);
	...
	H(), G(), X = class {
		...
		async initCache() {
			if (this.cache) return this.cache;
			{
				let e;
				{
					let { NodeJsTileFetcher: t } = await Promise.resolve().then(() => (ee(), J));
					e = new t();
				}
...
Q(), S();
var re = b("ElevationProvider"), $ = class {
	constructor(e = {}) {
		this.config = {
		...
		}, re.dir("Config :", this.config), this.validateConfig(), this.tileManager = new X(this.config.tileUrlTemplate, this.config.cacheSize), this.calculator = new p(this.tileManager, this.config.tileSize), this.batchCalculator = new F(this.calculator);
	}
	getConfig() {
		return { ...this.config };
	}
