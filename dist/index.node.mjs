import { ImageData as e } from "canvas";
import t from "sharp";
...
x();
var j = b("utils/ElevationSmoother"), M = class e {
	static smooth(t, n = 50) {
		if (j.debug("smooth %s", t.length), t.length < T.MIN_SMOOTHING_POINTS) return j.debug("too small"), t;
		if (n <= 0) throw Error(`Invalid window size: ${n}. Must be positive`);
		j.timeLevel(h.INFO, "smooth");
		let r = E.cumulativeDistances(t), i = e.smoothProfile(r, t.map((e) => e.elevation), n), a = t.map((e, t) => ({
			...e,
			elevation: i[t]
		}));
		return j.timeEndLevel(h.INFO, "smooth"), a;
	}
	static smoothProfile(e, t, n) {
		if (e.length !== t.length) throw Error(`distances (${e.length}) and elevations (${t.length}) must have the same length`);
		let r = t.length;
		if (r < T.MIN_SMOOTHING_POINTS || !(n > 0)) return Array.from(t);
		let i = Array(r), a = 0, o = 0;
		for (let s = 0; s < r; s++) {
			let c = e[s];
			for (; c - e[a] > n;) a++;
			for (o < s && (o = s); o < r - 1 && e[o + 1] - c <= n;) o++;
			let l = 0, u = 0;
			for (let r = a; r <= o; r++) {
				let i = 1 - Math.abs(e[r] - c) / n;
				l += i, u += t[r] * i;
			}
			i[s] = u / l;
		}
	...
	async getElevationsAlong(e, t, n, r, i, a, o) {
		...
		return o?.enabled === !0 && P.debug("Filtering skipped - insufficient points: %d (minimum: 3)", c.length), P.timeEndLevel(h.INFO, s), P.info("Path processing completed - waypoints: %d, final points: %d, smoothed: %s, filtered: %s", e.length, c.length, a?.enabled, o?.enabled), c;
	}
...
})), ee = /* @__PURE__ */ i({ NodeJsTileFetcher: () => J }), J, te = r((() => {
	q(), J = class {
		async fetchTile(n) {
			let r = await fetch(n);
			if (!r.ok) throw Error(`HTTP ${r.status}: ${r.statusText}`);
			let i = Buffer.from(await r.arrayBuffer()), { data: a, info: o } = await t(i).ensureAlpha().raw().toBuffer({ resolveWithObject: !0 });
			if (o.premultiplied || o.channels !== 4) throw Error(`Unexpected decode for ${n}: channels=${o.channels}, premultiplied=${o.premultiplied}`);
			return new K(new e(new Uint8ClampedArray(a), o.width, o.height));
		}
	};
})), Y, ne = r((() => {
	H(), G(), Y = class {
		...
		async initCache() {
			if (this.cache) return this.cache;
			{
				let e;
				{
					let { NodeJsTileFetcher: t } = await Promise.resolve().then(() => (te(), ee));
					e = new t();
				}
...
Z(), S();
var ie = b("ElevationProvider"), Q = class {
	constructor(e = {}) {
		this.config = {
		...
		}, ie.dir("Config :", this.config), this.validateConfig(), this.tileManager = new Y(this.config.tileUrlTemplate, this.config.cacheSize), this.calculator = new p(this.tileManager, this.config.tileSize), this.batchCalculator = new F(this.calculator);
	}
	getConfig() {
		return { ...this.config };
	}
	...
	static compute(t, n = {}) {
		return e.computeProfile(E.cumulativeDistances(t), t.map((e) => e.elevation), n);
	}
	static computeProfile(t, n, r = {}) {
		let { thresholdM: i, smoothWindowM: a } = e.resolveOptions(r);
		if (t.length !== n.length) throw Error(`distances (${t.length}) and elevations (${n.length}) must have the same length`);
		let o = M.smoothProfile(t, n, a), s = 0, c = 0;
		for (let e = 1; e < o.length; e++) {
			let t = o[e] - o[e - 1];
			t > 0 ? s += t : c -= t;
		}
		let l = i > 0 ? e.accumulate(o, i) : {
			gain: s,
			loss: c,
			legCount: 0
		};
	...
	static resolveOptions(e) {
		let t = e.preset ?? "dem", n = Object.prototype.hasOwnProperty.call($, t) ? $[t] : void 0;
		if (!n) throw Error(`Unknown elevation gain preset '${t}': expected one of ${Object.keys($).join(", ")}`);
		let r = e.thresholdM ?? n.thresholdM, i = e.smoothWindowM ?? n.smoothWindowM;
		if (!(Number.isFinite(r) && r >= 0)) throw Error(`thresholdM must be finite and >= 0, was ${r}`);
		if (!(Number.isFinite(i) && i >= 0)) throw Error(`smoothWindowM must be finite and >= 0, was ${i}`);
		return {
			thresholdM: r,
			smoothWindowM: i
		};
	...
	static accumulate(e, t) {
		let n = 0, r = 0, i = 0, a = e[0], o = e[0], s = 0, c = 0, l = 0, u = e[0], d = e[0];
		for (let f = 1; f < e.length; f++) {
			let p = e[f];
			l === 0 ? (p > a && (a = p, s = f), p < o && (o = p, c = f), a - o >= t && (s > c ? (l = 1, u = o, d = a) : (l = -1, u = a, d = o))) : l > 0 ? p > d ? d = p : d - p >= t && (n += d - u, i++, u = d, d = p, l = -1) : p < d ? d = p : p - d >= t && (r += u - d, i++, u = d, d = p, l = 1);
		}
		return l > 0 ? (n += d - u, i++) : l < 0 && (r += u - d, i++), {
			gain: n,
			loss: r,
			legCount: i
		};
...
//#endregion
export { $ as ELEVATION_GAIN_PRESETS, ae as ElevationGain, Q as ElevationProvider, Q as default, M as ElevationSmoother };
