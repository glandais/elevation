...
var x = {
...
}, C = { MIN_SMOOTHING_POINTS: 3 }, w = class e {
	...
	static toEcef(e, t = 3) {
		...
		return new ee((o + i) * s * c, (o + i) * s * l, (o * (1 - x.FIRST_ECCENTRICITY_SQUARED) + i) * a);
	}
...
y();
var O = v("utils/ElevationSmoother"), k = class e {
	static smooth(t, n = 50) {
		if (O.debug("smooth %s", t.length), t.length < C.MIN_SMOOTHING_POINTS) return O.debug("too small"), t;
		if (n <= 0) throw Error(`Invalid window size: ${n}. Must be positive`);
		O.timeLevel(p.INFO, "smooth");
		let r = w.cumulativeDistances(t), i = e.smoothProfile(r, t.map((e) => e.elevation), n), a = t.map((e, t) => ({
			...e,
			elevation: i[t]
		}));
		return O.timeEndLevel(p.INFO, "smooth"), a;
	}
	static smoothProfile(e, t, n) {
		...
		let r = t.length;
		if (r < C.MIN_SMOOTHING_POINTS || !(n > 0)) return Array.from(t);
	...
	async getElevationsAlong(e, t, n, r, i, a, o) {
		...
		return o?.enabled === !0 && j.debug("Filtering skipped - insufficient points: %d (minimum: 3)", c.length), j.timeEndLevel(p.INFO, s), j.info("Path processing completed - waypoints: %d, final points: %d, smoothed: %s, filtered: %s", e.length, c.length, a?.enabled, o?.enabled), c;
	}
...
})), K = /* @__PURE__ */ n({ BrowserTileFetcher: () => q }), q, J = t((() => {
	U(), G(), q = class {
		constructor() {
	...
	L(), B(), Y = class {
		...
		async initCache() {
			if (this.cache) return this.cache;
			{
				let e;
				{
					let { BrowserTileFetcher: t } = await Promise.resolve().then(() => (J(), K));
					e = new t();
				}
...
Z(), b();
var oe = v("ElevationProvider"), Q = class {
	constructor(e = {}) {
		this.config = {
		...
		}, oe.dir("Config :", this.config), this.validateConfig(), this.tileManager = new Y(this.config.tileUrlTemplate, this.config.cacheSize), this.calculator = new d(this.tileManager, this.config.tileSize), this.batchCalculator = new te(this.calculator);
	}
	getConfig() {
		return { ...this.config };
	}
	...
	static compute(t, n = {}) {
		return e.computeProfile(w.cumulativeDistances(t), t.map((e) => e.elevation), n);
	}
	static computeProfile(t, n, r = {}) {
		...
		let o = k.smoothProfile(t, n, a), s = 0, c = 0;
...
//#endregion
export { $ as ELEVATION_GAIN_PRESETS, se as ElevationGain, Q as ElevationProvider, Q as default, k as ElevationSmoother };
